-- =====================================================================
-- Calendario calcula o saldo projetado com a mesma memoria das colunas
-- =====================================================================
-- PROBLEMA: Recebimentos e Despesas eram lidos das projecoes vivas, mas o
-- Saldo caixa vinha do snapshot painel_fluxo_caixa. Uma alteracao de premissa
-- podia deixar as tres colunas em instantes diferentes.
--
-- SOLUCAO: mantem o saldo realizado ate o corte. Depois dele, acumula a mesma
-- entrada e saida que a RPC retorna: saldo anterior + recebimentos - despesas.
-- Nao altera lancamentos, pagamentos ou o snapshot; apenas o saldo futuro do
-- Calendario. Sem cron ou refresh adicional.
...
begin
  ...
  with dias as (
    ...
    where v.data_venda::date >= p_mes and v.data_venda::date < p_mes + interval '1 month'
    ...
    from public.raw_stone_recebiveis r
    where r.data_vencimento >= p_mes and r.data_vencimento < p_mes + interval '1 month'
    group by r.data_vencimento
  ), pix_recebido as (
    select e.data_hora::date as dia, sum(e.valor) as valor
    from public.raw_stone_extrato e
    where e.movimentacao = 'Crédito' and e.tipo = 'Pix'
      and e.data_hora::date >= p_mes and e.data_hora::date < p_mes + interval '1 month'
    ...
    where f.data_caixa >= p_mes and f.data_caixa < p_mes + interval '1 month'
      and f.movimentacao = 'Débito' and f.entra_dre
    ...
    where p.data_pagamento >= p_mes and p.data_pagamento < p_mes + interval '1 month'
    ...
    select d.dia, extract(isodow from d.dia)::smallint as dia_semana,
      ...
      coalesce(r.credito, 0) + coalesce(r.debito, 0) + coalesce(pr.valor, 0) + coalesce(rp.valor, 0) as recebimento_total,
      case when d.dia <= ct.caixa then coalesce(dr.total, 0)
           else coalesce(dfp.total, 0) + coalesce(ddp.total, 0) end as despesa_total,
      case when d.dia <= ct.caixa then least(coalesce(rr.total, 0), coalesce(dr.total, 0))
           else coalesce(dfp.total, 0) end as despesa_recorrente,
      case when d.dia <= ct.caixa then greatest(coalesce(dr.total, 0) - coalesce(rr.total, 0), 0)
           else coalesce(ddp.total, 0) end as despesa_nao_recorrente,
      coalesce(rr.total, 0) as despesa_recorrente_registrada,
      case when d.dia <= ct.caixa then greatest(coalesce(rr.total, 0) - coalesce(dr.total, 0), 0)
...
comment on function public.listar_calendario_financeiro(date) is
  'Calendario financeiro mensal: realizado ate o corte e saldo futuro calculado com os recebimentos e despesas exibidos.';
