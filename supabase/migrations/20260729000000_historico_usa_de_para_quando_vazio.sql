-- =====================================================================
-- Historico passa a usar DE-Para quando nao tem categoria (so os "ND")
-- =====================================================================
--
-- CONTEXTO
--   raw_historico (planilha legada) traz categoria/dre_grupo congelados no
--   momento da importacao. Diferente de stone_extrato/bb/bs_cash, essas
--   linhas nunca consultam de_para -- por isso, classificar uma excecao do
--   historico em classificar_excecoes.html (que grava em de_para) nao tinha
--   nenhum efeito sobre aquela linha especifica.
--
-- DECISAO (usuario, 2026-07-05)
--   Nao reclassificar retroativamente linhas do historico que ja tem
--   categoria (ex.: as 82 linhas "Folha Salarial" do CNPJ do BS Cash,
--   -R$801.488,52) -- isso fica congelado. A unica mudanca e liberar as
--   linhas SEM categoria (excecao) para seguir o fluxo normal de DE-Para.
--
-- MUDANCA
--   A CTE `historico` ganha um lookup em de_para (por cnpj e por nome, mesma
--   precedencia/normalizacao ja usada em live_match) que so e considerado
--   quando h.categoria is null. Linhas com categoria ja preenchida ignoram
--   esse lookup e continuam exatamente como estavam.
--
-- RISCO: baixo.
--   - Nenhuma linha com categoria ja preenchida muda de valor.
--   - Linhas hoje em 'excecao' sem match no de_para continuam 'excecao'.
--   - Linhas hoje em 'excecao' com match no de_para passam a 'classificado'
--     (ou 'analise', se o de_para tiver a marca ANALISAR INDIVIDUAL) --
--     esse e o efeito pretendido.
-- =====================================================================
...
historico_base as (
  select
    ...
    h.fornecedor as fornecedor_hist,
    h.categoria as categoria_hist,
    h.dre_grupo as dre_grupo_hist
...
historico_match as (
  ...
  left join de_para_u dpc
    on hb.categoria_hist is null
   and dpc.chave_tipo = 'cnpj'
   and dpc.chave_valor = case
     when hb.contraparte_doc like '%/%' and hb.contraparte_doc not like '%*%' then so_digitos(hb.contraparte_doc)
     else null
   end
  left join de_para_u dpn
    on hb.categoria_hist is null
   and dpn.chave_tipo = 'nome'
   and dpn.chave_valor = case
     when hb.contraparte_nome ilike 'desconhecido' then null
     else normaliza_nome(hb.contraparte_nome)
   end
),
historico as (
  select
    ...
    hm.raw_id,
    hm.empresa,
    hm.data_hora::date as data_caixa,
    hm.data_hora::date as data_competencia,
    hm.movimentacao,
    hm.tipo,
    hm.valor,
    hm.contraparte_nome,
    hm.contraparte_doc,
    coalesce(hm.fornecedor_hist, hm.dp_forn) as fornecedor,
    case
      when hm.categoria_hist is not null then hm.categoria_hist
      when hm.dp_cat = 'ANALISAR INDIVIDUAL' then null
      else hm.dp_cat
    end as categoria,
    case
      when hm.categoria_hist is not null then hm.dre_grupo_hist
      else cdh.dre_grupo
    end as dre_grupo,
    case
      when hm.categoria_hist is not null then 'classificado'
      when hm.dp_cat = 'ANALISAR INDIVIDUAL' then 'analise'
      when hm.dp_cat is not null then 'classificado'
      else 'excecao'
    end as status
  from historico_match hm
  left join categoria_dre cdh on cdh.categoria = hm.dp_cat
),
live_base as (
  select
