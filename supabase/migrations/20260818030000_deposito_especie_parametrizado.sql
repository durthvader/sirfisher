-- Parametriza unidade, conta, data inicial, fuso, descricao, lote e janelas da
-- conferencia de deposito em especie. As colunas publicas permanecem iguais.
...
with cfg as (
  ...
  from public.raw_bb b cross join cfg
...
with cfg as (
  ...
  select coalesce(sum(b.valor), 0)::numeric(14, 2) as total,
         max(b.data) as ultimo_deposito
  from public.raw_bb b cross join cfg
  where b.lancamento ilike cfg.deposito_descricao_padrao
    and b.data >= cfg.inicio - greatest(
      0, public.parametro_valor('deposito_janela_anterior_dias', 1)::integer
    )
    and (cfg.conta_deposito_id is null or b.conta_id = cfg.conta_deposito_id)
), extrato_ate as (
  select max(b.data) as ate
  from public.raw_bb b cross join cfg
  where cfg.conta_deposito_id is null or b.conta_id = cfg.conta_deposito_id
), ajuste as (
...
comment on view public.app_conferencia_deposito_especie is
  'Lotes de deposito da unidade principal confrontados com a conta e as janelas configuradas.';
