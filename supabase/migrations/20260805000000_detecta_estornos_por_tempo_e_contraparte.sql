-- =====================================================================
-- Detecta estornos e pagamentos devolvidos sem reclassificar movimentos
-- =====================================================================
--
-- REGRA
--   Procura, na mesma conta importada e no mesmo dia, um debito seguido de
--   credito de mesmo valor absoluto. Documento/nome da contraparte e palavras
--   como estorno/devolvido aumentam a confianca.
--
-- NIVEIS
--   forte    = par unico, evidencia de contraparte/texto e ate 60 minutos;
--   provavel = par unico e evidencia, mas sem horario ou acima de 60 minutos;
--   analise  = mesmo valor no dia, mas evidencia insuficiente ou ambigua.
--
-- BB e Inter nao fornecem horario na base atual. Seus pares nunca recebem o
-- nivel forte. A rotina e somente diagnostica e nao grava ajuste_manual.
-- =====================================================================
...
pares_reversao_base as (
  ...
  join fatos c
   ...
   and d.valor < 0
   and c.valor > 0
   and abs(abs(d.valor) - abs(c.valor)) <= 0.01
   and c.data_caixa = d.data_caixa
   and (
     d.data_hora is null
     or c.data_hora is null
     or c.data_hora between d.data_hora and d.data_hora + interval '1 day'
   )
),
pares_reversao as (
  select
    p.*,
    case
      when p.qtd_para_debito = 1
       and p.qtd_para_credito = 1
       and (p.contraparte_confere or p.tem_indicio_textual)
       and p.minutos_intervalo between 0 and 60
        then 'forte'
      when p.qtd_para_debito = 1
       and p.qtd_para_credito = 1
       and (p.contraparte_confere or p.tem_indicio_textual)
  ...
  where p.ordem_debito = 1
    and p.ordem_credito = 1
),
reversoes_por_perna as (
  select
    p.debito_origem as origem,
    p.debito_raw_id as raw_id,
    p.credito_origem as par_origem,
    p.credito_raw_id as par_raw_id,
    p.nivel_reversao,
    p.evidencia_reversao,
    p.minutos_intervalo,
...
base as (
  select
    f.*,
    r.nivel_reversao,
    r.evidencia_reversao,
    r.minutos_intervalo,
    r.contraparte_confere,
    r.qtd_candidatos as qtd_candidatos_reversao,
    r.par_origem as reversao_par_origem,
    r.par_raw_id as reversao_par_raw_id,
    (
      f.categoria in (
  ...
  from fatos f
  left join reversoes_por_perna r
    on r.origem = f.origem
   and r.raw_id = f.raw_id
...
resultado as (
  select
    ...
    p.par_origem as contabil_par_origem,
    p.par_raw_id as contabil_par_raw_id,
    p.par_data as contabil_par_data,
    p.par_data_hora as contabil_par_data_hora,
    p.par_valor as contabil_par_valor,
    p.par_categoria as contabil_par_categoria,
    p.par_contraparte as contabil_par_contraparte,
    p.dias_diferenca,
    rp.data_caixa as reversao_par_data,
    rp.data_hora as reversao_par_data_hora,
    rp.valor as reversao_par_valor,
    rp.categoria as reversao_par_categoria,
    rp.contraparte_nome as reversao_par_contraparte
  from base b
  left join principal_contabil p
    on p.origem = b.origem
   and p.raw_id = b.raw_id
  left join fatos rp
    on rp.origem = b.reversao_par_origem
   and rp.raw_id = b.reversao_par_raw_id
)
select
  r.origem,
  r.raw_id,
  ...
  r.contraparte_confere,
  (r.data_hora is not null) as possui_horario,
...
comment on materialized view public.mv_conciliacao_contabil is
  'Auditoria de movimentos contabeis e possiveis estornos por valor, conta, contraparte e intervalo; nunca reclassifica dados.';
...
comment on view public.app_conciliacao_contabil is
  'Detalhe protegido da conciliacao contabil e da deteccao de possiveis estornos.';
